...
if (isset($data['user_id']) && isset($data['tasks'])) {
    ...
    // Bereite das Statement zum Löschen von Aufgaben vor
    $stmt = $conn->prepare("DELETE FROM tasks WHERE user_id = ? AND id = ?");
    ...
    try {
        foreach ($tasks as $task) {
            // Überprüfe, ob die Aufgabe die ID hat
            if (isset($task['id'])) {
                $id = (int)$task['id'];
        ...
        echo json_encode(['error' => 'Fehler beim Löschen der Aufgaben: ' . $e->getMessage()]);
    }
